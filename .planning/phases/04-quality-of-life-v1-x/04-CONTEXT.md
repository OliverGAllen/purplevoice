...
- **D-02:** **Re-paste hotkey = `cmd+shift+v`.** Pairs visually with `cmd+v` (system paste). Risk acknowledged: collides with macOS "Paste and Match Style" in some apps — but PurpleVoice's paste IS plain text (transient marker per Phase 2 INJ-03), so the collision is semantically aligned. Plan-level review may revisit if a specific app's collision proves frustrating.
  ...
  **D-02-SUPERSEDED (2026-05-01):** during live walkthrough sign-off, `cmd+shift+v` exhibited an opaque clipboard-manager collision: Hammerspoon's `hs.hotkey.bind` returned a truthy hotkey object (no binding-failed alert at load), but the keystroke never reached `repaste()`. Some system-level clipboard manager (Raycast / Maccy / Paste / similar — exact owner not identified) registered the chord first via Carbon `RegisterEventHotKey` and silently consumed every press. Combined with the already-documented VS Code/Cursor "Markdown Preview" cost, the hotkey was changed to **F18 via Karabiner backtick-hold**: `hs.hotkey.bind({}, "f18", repaste)` paired with `assets/karabiner-backtick-to-f18.json` (`to_if_alone` types backtick, `to_if_held_down` 200ms emits F18). F18 has zero known collisions — no commonly-used app binds bare F18. Backtick remains usable for typing via the `to_if_alone` clause. Key-code is `non_us_backslash` (Oliver's UK keyboard layout); ANSI/US would need `grave_accent_and_tilde`.
