...
**Requirements**: DST-01, DST-02, DST-03, DST-04, **DST-05 (TBD — public one-line installer)**, **DST-06 (TBD — Hammerspoon-as-PurpleVoice wrapping decision; A/B/C trade-off; user-surfaced 2026-04-30 during Phase 3.5 testing)**
**Success Criteria** (what must be TRUE):
...
| 7 | Phase 3: Distribution & Benchmarking + Public Install | 1/5 | In Progress 2026-05-01 — Plan 03-00 (Wave 0 staging) complete: 5 RED unit tests (test_install_sh_detection / no_init_lua_edit / dst06_option_b / uninstall_dryrun / license_present) + 4 walkthrough scaffolds (test_install_idempotent → DST-01, test_readme_recovery_walkthrough → DST-03, test_benchmark_run → DST-04, test_curl_bash_install → DST-05) + tests/benchmark/HOW-TO-REGENERATE.md staging the validation contract for Plans 03-01..04. Suite 11 PASS / 5 FAIL (intentional staging — RED tests reference install.sh / uninstall.sh / LICENSE which Plans 03-01..02 create). Plans 03-01..04 next: install.sh rename + Option B + LICENSE + uninstall + README rewrite + hyperfine + public flip | - |
