...
| 6 | Phase 4 (v1.x): Quality of Life | 1/3 | In Progress 2026-04-30 — Plan 04-00 staging complete (tests/test_karabiner_check.sh RED-at-Wave-0 with 8 string-level wiring assertions; 3 manual walkthrough scaffolds for QOL-01 + QOL-NEW-01 positive-path + Karabiner-missing negative-control; REQUIREMENTS.md QOL-01 promoted v2 → v1 with concrete language, QOL-NEW-01 added, v1 coverage 39 → 41, v2 QOL stubs rebranded to `purplevoice` namespace per D-10). 04-01 lua-core (cmd+shift+v re-paste + F19 binding) and 04-02 karabiner-docs (assets/karabiner-fn-to-f19.json + setup.sh Step 9 + README + SECURITY.md + REQUIREMENTS.md closure) remain. | - |
