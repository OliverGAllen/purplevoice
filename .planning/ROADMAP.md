...
**Created:** 2026-04-23
**Updated:** 2026-05-01 (Phase 3 Plan 03-01 closed 2/5 — install.sh rename + curl|bash bootstrap + DST-01 walkthrough sign-off live with 1 structural deviation accepted; DST-01/02/05/06 Complete; v1 coverage 41 → 43)
...
- [x] **Phase 3: Distribution & Benchmarking + Public Install** — Reproducible local install, public one-line installer, README, and `hyperfine` measurements that gate the v1.1 decision. **Reordered to end of v1 (2026-04-28) per user direction** — distribute the *finished, security-audited* product, not a half-polished one. Inherits SECURITY.md claims from Phase 2.7. (completed 2026-05-04)
- [x] **Phase 5 (v1.1, conditional): Warm-Process Upgrade** — DEFERRED 2026-05-04 after hyperfine numbers landed: 5s.wav p50/p95 = 0.589/0.605 s on M2 Max, well within budget. Cold-start pipeline is fast enough for v1.x; warm-process daemon not required. Pattern 2 (`transcribe()` boundary) preserved as latent drop-in swap point.
...
**Plans:** 6 plans
  ...
  - [x] 02.7-01-PLAN.md — Threat model (STRIDE + LINDDUN) + Scope (assets/trust boundaries) + tests/test_security_md_framing.sh D-17 lint (Wave 1, parallel with 02.7-02) *(complete 2026-04-29; commits eaac716, 0a0a58e; 02.7-01-SUMMARY.md)*
  - [x] 02.7-02-PLAN.md — Verification scripts (verify_egress 3-layer chain, verify_sbom, verify_air_gap, verify_signing) + setup.sh Step 8 SBOM regen with deterministic post-process (Wave 1, parallel with 02.7-01) *(complete 2026-04-29; commits cbba76c, 187802b, 2a5507a; 02.7-02-SUMMARY.md)*
  - [x] 02.7-03a-PLAN.md — Gap analysis: NIST SP 800-53 Rev 5 / Low-baseline (deep per-control mapping; 20 control rows — 5 above the 15 minimum — from AC, AU, IA, SC, SI, PT, CM, MP, SA, SR families) (Wave 2 — split from old 03 per checker M-5) *(complete 2026-04-29; commit b0f44e2; 02.7-03a-SUMMARY.md)*
  - [x] 02.7-03b-PLAN.md — Gap analysis: 6 framed framework sections (FIPS 140-3 / FedRAMP-tailored / Common Criteria / HIPAA / SOC 2 / ISO 27001) with disclaimer-first Pitfall-11 framing (Wave 3 — split from old 03 per checker M-5; sequential with 03a to avoid SECURITY.md merge conflicts)
  - [x] 02.7-04-PLAN.md — Documentation finalisation: SECURITY.md complete (Tasks 4-1a navigation + 4-1b verification cross-refs + 4-1c deferral framing — TL;DR + Audience Entry-Points + How to Verify + Egress Verification + SBOM + Air-Gap + Code Signing + Reproducible Build + Vuln Disclosure) + verify_reproducibility.sh impl + REQUIREMENTS.md SEC-01..06 formalisation + README.md expansion (Wave 4)
...
**Requirements**: DST-01, DST-02, DST-03, DST-04, DST-05 (formalised 2026-05-01 in Plan 03-01 closure — public one-line installer via curl|bash + bootstrap_clone_then_re_exec), DST-06 (formalised 2026-05-01 in Plan 03-01 closure — Hammerspoon-as-PurpleVoice wrapping decision = Option B bundled installer, no fork, per CONTEXT.md D-01)
**Success Criteria** (what must be TRUE):
  ...
  6. **DST-06 — Hammerspoon-as-PurpleVoice wrapping decision documented and implemented.** During `/gsd:discuss-phase 3`, weigh three options the user surfaced 2026-04-30:
     - **Option A — Custom Hammerspoon fork**: change bundle ID `org.hammerspoon.Hammerspoon` → `app.purplevoice.PurpleVoice`, change app name + icon, ship as `PurpleVoice.app`. *Effort: ~1-2 weeks + ongoing maintenance burden tracking upstream.* TCC permissions rebind to the new bundle ID (users grant Mic + Accessibility fresh on first launch). Requires Apple Developer ID + notarisation pipeline (Phase 2.7 SEC-04 deferred this — Phase 3 is the natural home). License compliance: Hammerspoon is BSD-3-clause (LuaSkin) + MIT (extensions) — repackaging allowed with attribution.
...
| 3 | Phase 2.5: Branding | 3/4 | Complete    | 2026-05-04 |
...
| 7 | Phase 3: Distribution & Benchmarking + Public Install | 5/5 | Complete 2026-05-04 — All 5 plans landed. Plan 03-00 (Wave 0 staging) + 03-01 (install.sh rename + curl\|bash bootstrap + DST-01 walkthrough) + 03-02 (LICENSE + uninstall.sh + README D-11/D-12 rewrite + DST-03 walkthrough) + 03-03 (hyperfine harness + reference WAVs + BENCHMARK.md template; Task 3-5 live walkthrough DEFERRED to BACKLOG#2 per Oliver — harness ready, benchmark execution pending Oliver's hardware time) + 03-04 (SECURITY.md "Distribution model" H3 + INSTALL_TOKEN soft-gate + public flip + DST-05 walkthrough). **Repo flipped PUBLIC** at https://github.com/OliverGAllen/purplevoice (visibility=PUBLIC, MIT licensed, 118+ commits pushed to origin/main; anonymous curl serves install.sh with HTTP 200). DST-01..03, DST-05, DST-06 [x] Complete in REQUIREMENTS.md (v1 coverage 42/43 = 97.7%); DST-04 stays Pending/DEFERRED per BACKLOG#2 (resume recipe: `brew install hyperfine && bash tests/benchmark/run.sh` — produces numbers + Phase 5 verdict in ~3 min on AC power). Phase 5 trigger verdict stays "Conditional" until DST-04 lands. Plan 03-04 introduced the **INSTALL_TOKEN soft-gate** (SHA256 baked into install.sh; honestly framed in SECURITY.md §"Distribution model — Install gate" as a request-channel signal, not access control — public source means a determined party can bypass). Walkthrough sign-off commits: 191e4af (DST-01) + e8bd7bf (DST-03) + DST-04 deferred (b2f87f7 BACKLOG#2) + DST-05 walkthrough sign-off. Suite at close: functional 16/0; security 5/0; brand + framing lints GREEN; Pattern 2 invariant intact. | 2026-05-04 |
| 8 | Phase 5 (v1.1, conditional): Warm-Process Upgrade | 0/0 | **Trigger evaluated 2026-05-04: DEFERRED** — 5s.wav p50 0.589s ≤ 2s AND p95 0.605s ≤ 4s on M2 Max (~3.4× and ~6.6× margin); cold-start pipeline within budget. Pattern 2 (`transcribe()` boundary in purplevoice-record) preserved as latent drop-in swap point if future hardware/model crosses the trigger thresholds. | DEFERRED 2026-05-04 |
...
| 3. Distribution & Benchmarking + Public Install | DST-01, DST-02, DST-03, DST-04, DST-05, DST-06 | 6 |
| 3.5. Hover UI / HUD | HUD-01, HUD-02, HUD-03, HUD-04 | 4 |
| 4 (v1.x). Quality of Life | QOL-01, QOL-NEW-01 | 2 |
| **Total v1** | | **43** (was 26 → 29 with BRD → 35 with SEC → 39 with HUD → 41 with QOL → 43 with DST-05 + DST-06 formalised in Plan 03-01) |
...
*Roadmap updated: 2026-05-01 — Phase 3 Plan 03-01 closure: install.sh rename (was setup.sh) + Step 0 curl-vs-clone detection + bootstrap_clone_then_re_exec + D-13 GitHub-owner casing typo sweep (oliverallen → OliverGAllen) + brand-consistency exemption update + SBOM regeneration mid-walkthrough (orchestrator-applied D-01 fix) + DST-01 idempotency walkthrough signed off live by Oliver 5/6 PASS. Criterion 8 (SBOM zero git diff) DEFERRED-structural — install.sh:472-478 derives documentNamespace from `git rev-parse HEAD`, creating a circular self-reference (committing the regenerated SBOM moves HEAD, making the just-committed SBOM 1-commit stale relative to its own self-reference). Run 1 ↔ Run 2 byte-identity (md5 a48aae374ddb2ff908f6eade99282be8) proves DST-01 idempotency intent upheld. Backlog item filed for Phase 5 / v1.1: rewrite documentNamespace derivation to use `git rev-list -1 HEAD -- ':!SBOM.spdx.json'` or a static milestone tag. New deviation class added to library: "mathematically-unsatisfiable validation criterion accepted as DEFERRED-structural when underlying intent is provably upheld by alternative measurement." DST-01 + DST-02 + DST-05 + DST-06 marked Complete in REQUIREMENTS.md (v1 subsection + traceability table); v1 coverage 41 → 43. Phase 3 progress 1/5 → 2/5; Plan 03-02 unblocked.*
*Roadmap updated: 2026-05-04 — Phase 3 closed 5/5 + repo flipped PUBLIC. Plan 03-02 (LICENSE + uninstall.sh + README D-11/D-12 rewrite + DST-03 walkthrough — orchestrator ran TCC reset + uninstall + reinstall on Oliver's machine, all 4 README recovery items PASS) + Plan 03-03 (hyperfine harness + 3 reference WAVs + BENCHMARK.md template — Task 3-5 live walkthrough DEFERRED to BACKLOG#2 per Oliver, no time pressure) + Plan 03-04 (SECURITY.md "Distribution model" H3 + INSTALL_TOKEN soft-gate + public flip + DST-05 walkthrough). Repo at https://github.com/OliverGAllen/purplevoice flipped from PRIVATE to PUBLIC via `gh repo edit OliverGAllen/purplevoice --visibility public --accept-visibility-change-consequences`; 118 commits pushed to origin/main; anonymous curl-fetch of raw.githubusercontent.com/.../install.sh returns HTTP 200 + bash content. INSTALL_TOKEN soft-gate landed (Plan 03-04 commit 4d43035): SHA256 baked into install.sh; PURPLEVOICE_TEST_BYPASS_TOKEN_CHECK=1 escape for verify_air_gap.sh; honestly framed in SECURITY.md §"Distribution model — Install gate" as a request-channel signal (not access control — public source means a determined party can read install.sh + remove the gate; purpose is filtering casual installs + creating a "ping Oliver" channel). v1 coverage 42/43 (97.7%) — DST-01..03, DST-05, DST-06 [x] Complete; DST-04 [ ] Pending/DEFERRED with annotation pointing at BACKLOG#2. Phase 5 trigger verdict stays "Conditional" pending DST-04. New deviation classes added to library: (a) "destructive walkthrough item run by orchestrator on user's machine — user retains GUI re-grant + post-condition verification authority"; (b) "informational walkthrough findings that validate (rather than deviate from) the documented diagnostic flow are recorded in walkthrough doc + SUMMARY § Live findings, NOT as plan deviations"; (c) "soft access-control gate on public source — honest framing acknowledges public-readability bypassability; purpose is request-channel norm, not control surface". Phase 4 CHECKPOINT-3 (sudo-mv DEFERRED) precedent applied to DST-04. v1 publicly shipping.*
