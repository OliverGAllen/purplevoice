...
### Scope disclaimer: repo-only Syft scan

The committed `SBOM.spdx.json` enumerates **one** SPDX `package` entry — `PurpleVoice` itself. This is deliberate. Syft is invoked with `dir:$REPO_ROOT` scope only (Plan 02.7-02 setup.sh Step 8) — a naïve `dir:$HOME` scan would leak every Homebrew formula and unrelated git checkout on the host machine into the SBOM, which is both privacy-leaky and audit-noisy. The transitive dependencies named above (Karabiner-Elements (kernel-extension-class daemon for the fn→F19 hotkey remap; user-installed), sox audio libs, whisper.cpp / ggml internals, Hammerspoon's bundled Lua + LuaSocket) are **carried by reference**, not enumerated as separate `package` entries: the four SPDX 2.3 Annotation blocks pin the host OS / hardware / Xcode CLT / brew versions, and `setup.sh` itself pins each binary by SHA256 (`MODEL_SHA256` for `ggml-small.en.bin`, `SILERO_SHA256` for `ggml-silero-v6.2.0.bin`, brew bottle hashes for `sox` / `whisper-cpp` / `Hammerspoon.app` resolved through the formula manifests at `https://formulae.brew.sh/api/formula/{sox,whisper-cpp}.json` and `https://formulae.brew.sh/api/cask/hammerspoon.json`). Auditors who require a single document enumerating every transitive dependency at the package level should consult those upstream Homebrew SBOMs in conjunction with this one. This scope choice is honest about the tradeoff: PurpleVoice's repo is the surface PurpleVoice authors control; the brew-installed dependencies have their own upstream provenance trail that PurpleVoice does not duplicate. A future opt-in `tests/security/verify_sbom_full.sh` extension could perform a multi-scope scan (`/opt/homebrew/Cellar/{sox,whisper-cpp}` + `/Applications/Hammerspoon.app`) on demand for institutional adopters who need the consolidated view; it is not committed to the v1 release.
