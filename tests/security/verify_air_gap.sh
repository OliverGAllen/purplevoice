...
# Asserts: PURPLEVOICE_OFFLINE=1 mode in install.sh is honoured.
#   Invariant 1: with PURPLEVOICE_OFFLINE=1 + all sideload artefacts present,
#                install.sh exits 0 (no network calls).
#   Invariant 2: with PURPLEVOICE_OFFLINE=1 + Whisper model missing,
#                install.sh exits 1 with actionable error message containing
...
# -------------------------------------------------------------------------
if [ ! -f "$MODEL" ]; then
  fail "pre-flight: $MODEL not present — run install.sh (online) before verify_air_gap.sh"
fi

# -------------------------------------------------------------------------
# Pre-flight: Karabiner-Elements.app must exist (Phase 4 / QOL-NEW-01 — install.sh
...
if [ ! -d /Applications/Karabiner-Elements.app ]; then
  echo "  verify_air_gap.sh: Karabiner-Elements.app absent — skipping install.sh exec invariants."
...
if PURPLEVOICE_OFFLINE=1 PURPLEVOICE_TEST_BYPASS_TOKEN_CHECK=1 bash install.sh > "$LOG_OFFLINE_OK" 2>&1; then
  ...
  fail "Invariant 1: PURPLEVOICE_OFFLINE=1 install.sh exited non-zero with sideload populated. Log: $LOG_OFFLINE_OK"
fi

# -------------------------------------------------------------------------
# Invariant 2: PURPLEVOICE_OFFLINE=1 + missing model → install.sh exits 1 with
...
if [ "$EXIT_CODE" -eq 0 ]; then
  fail "Invariant 2: PURPLEVOICE_OFFLINE=1 install.sh exited 0 with model missing (should exit 1)"
fi

if ! grep -q "PURPLEVOICE_OFFLINE=1 set but Whisper model not sideloaded" "$LOG_OFFLINE_MISSING"; then
  fail "Invariant 2: actionable error message missing from install.sh stderr. Log: $LOG_OFFLINE_MISSING"
fi
