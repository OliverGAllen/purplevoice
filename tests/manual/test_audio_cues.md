...
- Sounds still play with VOICE_CC_NO_SOUNDS=1 → env var not read at module load; check `os.getenv("VOICE_CC_NO_SOUNDS")` runs once at module init (NOT inside the press handler — `launchctl setenv` only affects newly-spawned processes, so a module load is required to pick it up).
